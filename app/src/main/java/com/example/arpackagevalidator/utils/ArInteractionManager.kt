...
class ArInteractionManager(
    ...
    fun drawState(state: MeasurementUiState) {
        allAssetsFuture.thenRun {
            ...
            Log.e("ArInteractionManager", "Gagal memuat aset AR.", it)
            null
        }
    }
    ...
    private fun createAnchorAt(position: Vector3): AnchorNode {
        ...
        val pose = com.google.ar.core.Pose(floatArrayOf(position.x, position.y, position.z), floatArrayOf(0f, 0f, 0f, 1f))
    ...
    private fun drawLine(p1: Vector3, p2: Vector3, material: Material, distance: Float) {
        ...
        val position = Vector3.add(p1, difference.scaled(0.5f))
        val rotation = Quaternion.lookRotation(difference, Vector3.up())
        val lineModel = ShapeFactory.makeCube(Vector3(LINE_THICKNESS, LINE_THICKNESS, difference.length()), Vector3.zero(), material)
    ...
    private fun drawPreviewBox(state: MeasurementUiState) {
        ...
        val p0 = state.points[0]; val p1 = state.points[1]; val p2 = state.points[2]
        val vecLength = Vector3.subtract(p1, p0); val vecWidth = Vector3.subtract(p2, p1)
        val center = Vector3.add(p0, vecLength.scaled(0.5f)).add(vecWidth.scaled(0.5f)).add(Vector3(0f, state.height / 200f, 0f))
    ...
    private fun addDistanceLabel(parentNode: Node, distance: Float) {
        labelRenderable.thenAccept { renderable ->
            ...
            addNodeToScene(labelView, parent = parentNode, localPosition = Vector3(0f, LABEL_Y_OFFSET, 0f))
        }
    }
